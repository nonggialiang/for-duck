...
// IsolatedClassLoader (gravitino-core), UserGroupInformation (Kerberos)
...
public class IcebergCatalogWrapper implements AutoCloseable {
  ...
  public void close() throws Exception {
    ...
    if (catalog instanceof AutoCloseable) {
      // JdbcCatalog implements AutoCloseable and will handle its own cleanup
      ((AutoCloseable) catalog).close();
    }
    metadataCache.close();
    // uses a single classloader, no driver deregistration needed.
  }

  // closeMySQLCatalogResource(), closeDriverLoadedByIsolatedClassLoader(),
  // closePostgreSQLCatalogResource() — all depended on IsolatedClassLoader
  // (gravitino-core), not needed in standalone server.
