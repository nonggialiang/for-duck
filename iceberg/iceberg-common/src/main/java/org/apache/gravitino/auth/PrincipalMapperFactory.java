...
/** Factory for creating {@link PrincipalMapper} instances. */
public class PrincipalMapperFactory {

  private PrincipalMapperFactory() {}

  /**
   * Creates a principal mapper.
   *
   * @param mapperType {@code regex} for the built-in regex mapper, or a fully qualified class name
   *     for a custom mapper
   * @param regexPattern the regex pattern (only used when {@code mapperType} is {@code regex})
   * @return a configured {@link PrincipalMapper}
