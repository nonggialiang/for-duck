...
public class IcebergExceptionMapper {
  ...
  public static int getErrorCode(Exception ex) {
    return EXCEPTION_ERROR_CODES.getOrDefault(ex.getClass(), HttpStatus.INTERNAL_SERVER_ERROR.value());
  }
  ...
  /** Converts a throwable to a Spring {@link ResponseEntity} with the appropriate status and body. */
  public static ResponseEntity<Object> toResponseEntity(Throwable ex) {
    int status =
        EXCEPTION_ERROR_CODES.getOrDefault(
            ex.getClass(), HttpStatus.INTERNAL_SERVER_ERROR.value());
    ...
    return HttpResponseBuilder.errorEntity(ex, status);
  }
}
