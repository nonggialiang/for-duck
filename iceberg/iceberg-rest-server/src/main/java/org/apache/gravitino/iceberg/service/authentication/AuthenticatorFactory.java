...
/** Creates {@link Authenticator} instances from {@link IcebergConfig#AUTHENTICATORS}. */
public class AuthenticatorFactory {

  private static final Logger LOG = LoggerFactory.getLogger(AuthenticatorFactory.class);

  public static final String KERBEROS_ALIAS = "kerberos";

  public static final ImmutableMap<String, String> AUTHENTICATORS =
      ImmutableMap.of(
          "simple", SimpleAuthenticator.class.getCanonicalName(),
          "oauth", OAuth2TokenAuthenticator.class.getCanonicalName());

  private AuthenticatorFactory() {}

  public static List<Authenticator> createAuthenticators(Config config) {
    List<String> authenticatorNames = config.get(IcebergConfig.AUTHENTICATORS);

    List<Authenticator> authenticators = Lists.newArrayList();
    for (String name : authenticatorNames) {
      if (KERBEROS_ALIAS.equalsIgnoreCase(name)) {
        throw new IllegalArgumentException(
            "Kerberos authentication is not supported by the Iceberg REST server");
      }
      String className = AUTHENTICATORS.getOrDefault(name.toLowerCase(), name);
      try {
