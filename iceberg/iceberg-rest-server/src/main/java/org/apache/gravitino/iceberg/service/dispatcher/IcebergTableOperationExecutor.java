...
public class IcebergTableOperationExecutor implements IcebergTableOperationDispatcher {
  ...
  private static CredentialPrivilege getCredentialPrivilege(
    ...
    String schema = tableIdentifier.namespace().levels().length > 0
        ? tableIdentifier.namespace().level(tableIdentifier.namespace().length() - 1) : "";
    IcebergResource resource =
        IcebergResource.ofTable(context.catalogName(), schema, tableIdentifier.name());
    CredentialPrivilege privilege = authorizer.checkCredential(context.userName(), resource);
    return privilege != null ? privilege : CredentialPrivilege.READ;
  }
